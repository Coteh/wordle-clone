...
describe("theme", () => {
    ...
    it("should apply dimmed theme-color for new theme when switching themes while dialog is open", () => {
        // Open settings, keep it open (it's not a dialog, it's a separate pane)
        cy.get(".settings-link").click();
        ...
        // Keep settings open, open help dialog on top
        cy.get(".settings .close").click();
        cy.get(".help-link").click();
        
        // Should have dimmed light color
        cy.get("meta[name='theme-color']").should("have.attr", "content", "#808080");
        
        // Close dialog, go back to settings
        cy.get(".dialog .close").click();
        cy.get(".settings-link").click();
        
        // Switch to snow theme while settings is open
        cy.get(".setting.theme-switch").click();
        cy.get("body").should("have.class", "snow");
        cy.get("meta[name='theme-color']").should("have.attr", "content", "#020024");
        
        // Close settings, open help dialog
        cy.get(".settings .close").click();
        cy.get(".help-link").click();
        
        // Should have dimmed snow color
        cy.get("meta[name='theme-color']").should("have.attr", "content", "#010012");
    });
});
